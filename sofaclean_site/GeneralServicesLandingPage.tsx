...
import { ArrowRight, Check, ChevronDown, Mail, MessageCircle, Phone, Sparkles, ShieldCheck } from 'lucide-react';
...
export function GeneralServicesLandingPage({ content: c, demo = false, onOpenContact }: { content: GeneralServicesContent; demo?: boolean; onOpenContact?: (topic?: string) => void }) {
  ...
  return <div className="template">
    ...
    <header className="nav shell"><div className="brand" aria-label={`${c.name} — início`}><span className="brand-logo"><Image unoptimized src="/sofaclean/logos/sofa-clean-logo.png" alt="SofaClean" width={180} height={72} /></span><span><strong>{c.name}</strong><b>{c.tagline}</b></span></div><nav aria-label="Navegação de serviços"><a href="#servicos">Serviços</a><a href="#processo">Como funciona</a><a href="#faq">FAQ</a></nav><div className="nav-actions"><a className="nav-call" href="#contacto" onClick={(event) => { event.preventDefault(); onOpenContact?.('Higienização de estofos'); }}><Mail size={17} aria-hidden="true" />Contactos</a><a className="nav-phone" href="tel:+351920320174"><Phone size={17} aria-hidden="true" />Ligar</a></div></header>
    <main id="servicos-conteudo">
      ...
      <section className="final-cta" id="orcamento"><div className="shell"><div className="kicker light">VAMOS CUIDAR DO SEU ESPAÇO</div><h2>Em que podemos<br />ajudar?</h2><p>Conte-nos o que precisa e peça uma proposta para o seu serviço.</p><div className="hero-actions centered"><a className="button green" {...contactProps('Higienização de estofos')}><MessageCircle aria-hidden="true" />Pedir orçamento</a>{canCall && <a className="button ghost" href={phone}><Phone aria-hidden="true" />{c.phone}</a>}</div><p className="final-social">Siga-nos e conheça mais sobre o nosso trabalho no <a href="https://www.instagram.com/sofaclean.porto" target="_blank" rel="noopener noreferrer">Instagram</a>.</p></div></section>
    </main>
