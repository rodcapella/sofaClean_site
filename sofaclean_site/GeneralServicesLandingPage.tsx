...
export function GeneralServicesLandingPage({ content: c, demo = false, onOpenContact }: { content: GeneralServicesContent; demo?: boolean; onOpenContact?: (topic?: string) => void }) {
  ...
  const emailContact = (subject: string) => ({ href: `mailto:sofaclean.porto@gmail.com?subject=${encodeURIComponent(subject)}` });
  ...
  return <div className="template">
    <header className="nav shell"><a className="brand brand-logo-crop" href="#top" aria-label={`${c.name} — início`}><Image unoptimized src="/sofaclean/logos/sofa-clean-logo.png" alt={c.name} width={220} height={110} /></a><nav aria-label="Navegação de serviços"><a href="#servicos">Serviços</a><a href="#processo">Como funciona</a><a href="#faq">FAQ</a><a href="#contacto" onClick={event => { event.preventDefault(); setContactOpen(true); }}>Contacto</a></nav></header>
    <main id="servicos-conteudo">
      <section className="hero" id="top"><div className="shell hero-grid"><div className="hero-copy"><div className="eyebrow">Serviços ao domicílio · {c.area}</div><h1>{c.title[0]}<br /><em>{c.title[1]}</em><br />{c.title[2]}</h1><p>{c.description}</p><div className="hero-actions"><a className="button primary" {...contactProps()}><MessageCircle size={20} aria-hidden="true" />Pedir orçamento <ArrowRight size={18} aria-hidden="true" /></a><a className="button secondary" href="#servicos">Conhecer os serviços</a></div><div className="trust-row"><span><Check aria-hidden="true" />{c.pricingHighlight}</span><span><Check aria-hidden="true" />{c.travelHighlight}</span></div></div>
      ...
      <section className="area section shell"><div className="area-card"><div><div className="kicker">ÁREA DE ATUAÇÃO</div><h2>Perto de si,<br />em {c.area}.</h2><p>Atendimento ao domicílio nas regiões do Porto e Braga. Partilhe a sua localização e confirme connosco a disponibilidade para a sua zona.</p><a className="button white" href={mapHref} target="_blank" rel="noreferrer"><MapPin aria-hidden="true" />Ver Porto e Braga no Google Maps</a></div><div className="coverage map-coverage"><MapPin size={52} aria-hidden="true" /><p className="map-regions">PORTO <span>·</span> BRAGA</p><strong className="map-travel">Deslocação gratuita nos arredores</strong><span>Confirme a cobertura da sua localidade.</span><span>{c.address}</span></div></div></section>
      ...
      <section className="final-cta" id="orcamento"><div className="shell"><div className="kicker light">VAMOS CUIDAR DO SEU ESPAÇO</div><h2>Em que podemos<br />ajudar?</h2><p>Conte-nos o que precisa e peça uma proposta para o seu serviço.</p><div className="hero-actions centered"><a className="button green" {...contactProps('Higienização de estofos')}><MessageCircle aria-hidden="true" />Pedir orçamento</a></div></div></section>
    </main>
    <SofaCleanFooter />
    <SofaCleanWhatsApp />
