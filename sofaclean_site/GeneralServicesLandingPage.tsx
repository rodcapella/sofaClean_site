...
export function GeneralServicesLandingPage({ content: c, demo = false, onOpenContact }: { content: GeneralServicesContent; demo?: boolean; onOpenContact?: (topic?: string) => void }) {
  ...
  return <div className={styles.template}>
    ...
    <header className="nav shell"><a className="brand" href="#top" aria-label={`${c.name} — início`}><span className="brand-mark"><Sparkles aria-hidden="true" /></span><span><strong>{c.name}</strong><b>{c.tagline}</b></span></a><nav aria-label="Navegação de serviços"><a href="#servicos">Serviços</a><a href="#processo">Como funciona</a><a href="#faq">FAQ</a></nav><a className="nav-call" href="#contacto" onClick={(event) => { event.preventDefault(); onOpenContact?.('Higienização de estofos'); }}><Phone size={17} aria-hidden="true" />Contactos</a></header>
    <main id="servicos-conteudo">
      ...
      <section className="area section shell"><div className="area-card"><div><div className="kicker">ÁREA DE ATUAÇÃO</div><h2>Perto de si,<br />em {c.area}.</h2><p>Deslocamo-nos diretamente até si nas zonas do Porto e de Braga. Confirme a cobertura da sua localidade connosco.</p><a className="button white" href={mapHref} target="_blank" rel="noreferrer"><MapPin aria-hidden="true" />Abrir no Google Maps</a></div><div className="coverage map-coverage"><div className="coverage-map" role="img" aria-label="Mapa das áreas de atendimento no Porto e em Braga"><iframe title="Mapa entre Porto e Braga" src="https://www.google.com/maps?q=Porto%20Braga%20Portugal&z=9&output=embed" loading="lazy" referrerPolicy="no-referrer-when-downgrade" /><span className="coverage-circle porto-circle" /><span className="coverage-circle braga-circle" /><span className="coverage-marker porto-marker">Porto</span><span className="coverage-marker braga-marker">Braga</span></div><p>Porto · Braga</p><span>Áreas de cobertura indicativas. Confirme a disponibilidade para a sua morada.</span><span>Deslocação gratuita nos arredores.</span><div className="outside-coverage"><h3>Precisa de serviços fora destas zonas?</h3><p>Entre em contacto connosco para avaliar a disponibilidade de deslocação para a sua localidade.</p><a className="button green" href="#" onClick={(event) => { event.preventDefault(); onOpenContact?.('Pedido de orçamento fora da área'); }}><MessageCircle aria-hidden="true" />Pedir orçamento grátis</a></div></div></div></section>
