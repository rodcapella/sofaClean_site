...
export function GeneralServicesLandingPage({ content: c, demo = false, onOpenContact }: { content: GeneralServicesContent; demo?: boolean; onOpenContact?: (topic?: string) => void }) {
  ...
  return <div className={styles.template}>
    ...
    <header className="nav shell"><a className="brand" href="#top" aria-label={`${c.name} — início`}><span className="brand-mark"><Sparkles aria-hidden="true" /></span><span><strong>{c.name}</strong><b>{c.tagline}</b></span></a><nav aria-label="Navegação de serviços"><a href="#servicos">Serviços</a><a href="#processo">Como funciona</a><a href="#faq">FAQ</a><a href="#contacto" onClick={event => { event.preventDefault(); setContactOpen(true); }}>Contacto</a></nav><a className="nav-call" href="#contacto" onClick={event => { event.preventDefault(); setContactOpen(true); }}><Phone size={17} aria-hidden="true" />{canCall ? c.phone : 'Contactos'}</a></header>
