...
export function LegalModal({ kind, onClose }: { kind: LegalKind; onClose: () => void }) {
  ...
  useEffect(() => { let active = true; setHtml(''); fetch(`${routes[kind]}?modal=1`, { headers: { Accept: 'text/html' } }).then(r => r.text()).then(text => { if (!active) return; const doc = new DOMParser().parseFromString(text, 'text/html'); const content = doc.querySelector('.legal-content') || doc.querySelector('main'); setHtml(content?.innerHTML || '<p>O conteúdo desta política está temporariamente indisponível.</p>'); }).catch(() => active && setHtml('<p>O conteúdo desta política está temporariamente indisponível.</p>')); return () => { active = false; }; }, [kind]);
