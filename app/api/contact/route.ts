...
export async function POST(request: Request) {
  ...
  const name=clean(body.name,100), email=clean(body.email,254), phone=clean(body.phone,30), subject=clean(body.subject,100), topic=clean(body.topic,100), message=clean(body.message,2000);
  ...
  const text = [`Nome: ${name}`, `Email: ${email}`, `Telefone: ${phone || '-'}`, `Assunto: ${subject || 'Contato'}`, `Serviço: ${topic}`, '', message].join('\n');
  const response = await fetch('https://api.resend.com/emails', { method: 'POST', headers: { Authorization: `Bearer ${apiKey}`, 'Content-Type': 'application/json' }, body: JSON.stringify({ from: process.env.CONTACT_FROM_EMAIL || 'SofaClean <onboarding@resend.dev>', to: ['sofaclean.porto@gmail.com'], reply_to: email, subject: `SofaClean — ${subject || topic}`, text }) });
